...
try {
    ...
    # Recheck after acquiring the mutex because another client process may have
    ...
    if (-not (Test-LiteLLM)) {
        ...
        if ($listeners.Count -gt 0) {
            throw "Port $($settings.Port) is already in use and the configured LiteLLM proxy is not responding. Stop the process using it, or rerun `"$setupScript`" with a free port (for example: -Port 4567), then restart the client."
        }
        ...
        if (-not $ready) {
            throw "LiteLLM did not become ready at $($settings.BaseUrl). See $logDirectory. If the port is occupied, rerun `"$setupScript`" with a free -Port value and restart the client."
        }
    }
